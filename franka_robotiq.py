...
class PandaRobotiq(BaseAgent):
    ...
    keyframes = dict(
        rest=Keyframe(
            qpos=np.array(
                [
                    # Seven arm joint positions (radians)
                    0.0,
                    0.685,
                    0,
                    1,
                    ...
                    1.5,
                    np.pi / 2,
                    # Six gripper joint positions (open configuration):
                    0.04,
                    0.04,
                    0.04,
                    0.04,
    ...
    def _controller_configs(self):
        ...
        # finger_joint_names = ["finger_joint",'right_outer_knuckle_joint']
        # finger_joint_names = ["right_outer_knuckle_joint"]
        # use a mimic controller config to define one action to control both fingers
        # -------------------------------------------------------------------------- #
        finger_mimic_pd_joint_pos = PDJointPosControllerConfig(
            finger_joint_names,
            lower=None,
            upper=None,
            stiffness=self.arm_stiffness,
        ...
        #         arm=arm_pd_joint_delta_pos, gripper=finger_mimic_pd_joint_pos
        #     ),
        #     pd_joint_pos=dict(arm=arm_pd_joint_pos, gripper=finger_mimic_pd_joint_pos),
            
        #     pd_ee_delta_pos=dict(arm=arm_pd_ee_delta_pos, gripper=finger_mimic_pd_joint_pos),
        #     pd_ee_delta_pose=dict(
        #         arm=arm_pd_ee_delta_pose, gripper=finger_mimic_pd_joint_pos
        #     ),
        #     pd_ee_pose=dict(arm=arm_pd_ee_pose, gripper=finger_mimic_pd_joint_pos),
        #     # TODO(jigu): how to add boundaries for the following controllers
        #     pd_joint_target_delta_pos=dict(
        #         arm=arm_pd_joint_target_delta_pos, gripper=finger_mimic_pd_joint_pos
        #     ),
        #     pd_ee_target_delta_pos=dict(
        #         arm=arm_pd_ee_target_delta_pos, gripper=finger_mimic_pd_joint_pos
        #     ),
        #     pd_ee_target_delta_pose=dict(
        #         arm=arm_pd_ee_target_delta_pose, gripper=finger_mimic_pd_joint_pos
        #     ),
        #     # Caution to use the following controllers
        #     pd_joint_vel=dict(arm=arm_pd_joint_vel, gripper=finger_mimic_pd_joint_pos),
        #     pd_joint_pos_vel=dict(
        #         arm=arm_pd_joint_pos_vel, gripper=finger_mimic_pd_joint_pos
        #     ),
        #     pd_joint_delta_pos_vel=dict(
        #         arm=arm_pd_joint_delta_pos_vel, gripper=finger_mimic_pd_joint_pos
        #     ),
        # )
        ...
        controller_configs = dict(
            ...
            pd_ee_delta_pos=dict(
                                ...
                                passive_gripper=passive_finger_joints
                                ),
            pd_ee_delta_pose=dict(
                                arm=arm_pd_ee_delta_pos,
                                gripper=finger_mimic_pd_joint_pos,
    ...
    def _after_init(self):
        self.finger1_link = sapien_utils.get_obj_by_name(
            self.robot.get_links(), "left_inner_knuckle"
        )
        self.finger2_link = sapien_utils.get_obj_by_name(
            self.robot.get_links(), "right_inner_knuckle"
        )
